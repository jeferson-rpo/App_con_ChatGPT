...
# Cargar y limpiar el DataFrame de deforestación
...
if opcion == "Cargar archivo desde URL":
    ...
    if url:
        df = pd.read_csv(url)  # Cargar el archivo CSV desde la URL
        st.write("Datos cargados desde la URL:", df)

elif opcion == "Subir archivo":
    archivo = st.file_uploader("Sube tu archivo CSV", type=["csv"])
    if archivo:
        df = pd.read_csv(archivo)  # Cargar el archivo CSV
...
if 'df' in locals():
    ...
    df[columnas_numericas.columns] = df[columnas_numericas.columns].fillna(df[columnas_numericas.columns].mean())

    # Limpiar las columnas de fechas
    columnas_fechas = df.select_dtypes(include=['object'])
    columnas_fechas = columnas_fechas.apply(pd.to_datetime, errors='coerce')
    fecha_promedio = columnas_fechas.mean()  # Promedio de las fechas
    df[columnas_fechas.columns] = columnas_fechas.fillna(fecha_promedio)  # Rellenar con la fecha promedio

    # Limpiar las columnas de texto
    columnas_texto = df.select_dtypes(include=['object'])
    for col in columnas_texto.columns:
        valor_frecuente = df[col].mode()[0]
        df[col] = df[col].fillna(valor_frecuente)
