...
def registrar_transaccion(df, fecha, tipo, categoria, monto, descripcion):
...
fecha_transaccion = st.date_input("Selecciona la fecha de la transacción:", datetime.date.today())

if st.button("Registrar Transacción"):
    if categoria_transaccion and monto_transaccion > 0:
        df = registrar_transaccion(df, fecha_transaccion.strftime("%Y-%m-%d"), 
                                   tipo_transaccion, categoria_transaccion, 
                                   monto_transaccion, descripcion_transaccion)
...
df["Fecha"] = pd.to_datetime(df["Fecha"])  # Asegurar que la columna Fecha es de tipo datetime
