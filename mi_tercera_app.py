...
def registrar_transaccion(df, tipo, categoria, monto, descripcion, fecha):
...
if st.button("Registrar Transacción"):
    if categoria_transaccion and monto_transaccion > 0:
        fecha_formateada = fecha_transaccion.strftime("%Y-%m-%d")
        df = registrar_transaccion(df, tipo_transaccion, categoria_transaccion, monto_transaccion, descripcion_transaccion, fecha_formateada)
