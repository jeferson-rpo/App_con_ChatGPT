...
if 'gdf' in locals():
    ...
    correlation_por_genero = gdf.groupby('Género')[['Edad', 'Ingreso_Anual_USD']].apply(lambda x: x.corr().iloc[0, 1])
    ...
    correlation_por_frecuencia = gdf.groupby('Frecuencia_Compra')[['Edad', 'Ingreso_Anual_USD']].apply(lambda x: x.corr().iloc[0, 1])
    ...
    ax.bar(correlation_por_genero.index, correlation_por_genero, color='g', label='Por Género')

    # Correlación por frecuencia de compra
    ax.bar(correlation_por_frecuencia.index, correlation_por_frecuencia, color='r', label='Por Frecuencia de Compra')
